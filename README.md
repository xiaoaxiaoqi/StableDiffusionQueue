# Stable Diffusion在线多人绘图(PHP)

## 项目介绍
* `当前项目仅供交流学习，请勿用于非法用途`
* 基于`FastAdmin`后台开发框架开发，下边有FastAdmin的相关介绍，感兴趣的可以阅读一下
* 解决了`Stable Diffusion` `WebUi` 多人绘图队列
* 添加了`用户管理系统`，支持用户注册、登录、余额、积分等功能
* 添加了简单的文生图用户绘画界面，支持提示词、图片比例、风格模型输入或选择
* 使用了GatewayWorker进行通知
## 运行环境
`宝塔面板` `PHP7.4` `Nginx 1.22.1` `MySQL 5.7.40` `Redis 7.0.11`
## 项目运行
* 添加站点，上传源代码
* 设置伪静态(选择thinkphp)、设置网站目录(public)
* 导入数据库文件，修改database.php(数据库名、用户名、密码)
* 修改`public/assets/js/frontend/user.js`20行ws地址
* 后台入口`域名/fekaDHRTEB.php`，账号：admin，密码：123qaz123
* 添加Stable Diffusion API端口：登录后台->左侧菜单栏(常规管理)->选项卡(端口配置)->键名(port1)， 键值(Stable Diffusion API端口) `PS：添加端口最后不要反斜杠/`
* 启动GatewayWorker：`php start.php start`
* 启动队列：`php think queue:listen --queue task`
* 启动端口：`php think queue:listen --queue port1`
* 执行命令行请在项目`根目录执行`
## 疑难解答QQ：858438539
